...
while(cap.isOpened()):
    ...
    if ret == True:
        ...
        if frame_num % 5 != 0:
            ...
            # IR Contour
            # Binary
            ret,image_c =cv2.threshold(image,150,255,cv2.THRESH_BINARY)
            # Contour
            contours, hierarchy = cv2.findContours(image_c,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
            image[:,:] = 0
            #image = np.zeros((256,192),np.uint8)
            ...
            #image = cv2.copyMakeBorder(image,10,115,52,32, cv2.BORDER_CONSTANT, value=[0,0,0])  #2.8mm Lens
            image = cv2.copyMakeBorder(image,15,60,45,90, cv2.BORDER_CONSTANT, value=[0,0,0])  #2.8mm Lens
